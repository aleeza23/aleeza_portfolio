              ...
              <div className="lg:pb-16">
                ...
                <ul className="flex flex-col gap-4 mt-4 md:mt-5">
                  {project.results.map(result => {
                    return <li key={result.title} className="flex gap-2 text-sm md:text-base text-white/50"><CheckIcon className='size-5 md:size-6' /><span>{result.title}</span></li>
                  })}
                </ul>

                <a href={project.link}>
                  <button className="bg-white inline-flex md:w-max px-6 justify-center items-center text-gray-950 h-12 rounded-xl mt-8 md:mt-[35.7px] w-full font-bold gap-2 "> <span>Visit Live Site</span><ArrowUp className='size-4' /></button>
                </a>
              </div>
